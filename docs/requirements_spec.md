...
| REQ-009 | 会议置顶 | 右键点击会议列表项 | SQLite is_pinned 字段 |
| REQ-010 | 会议搜索 | 左侧搜索框输入关键词 | SQLite LIKE 查询 |
| REQ-011 | 结束会议命名 | 会议结束时弹窗输入 | QInputDialog + 数据库更新 |
| REQ-012 | 会议时长显示 | 会议列表加载时计算 | Python 时间差计算 |
...
| 项目 | 描述 |
| --- | --- |
| 功能名称 | AI 智能摘要生成（按发言人分组） |
| 触发方式 | 用户点击右侧"生成摘要"按钮 |
| 输入 | 本次会议全部转写文本（格式："发言人1：内容\n发言人2：内容..."） |
| 输出 | 结构化 Markdown 摘要（按发言人分组核心观点、关键点 / 待办事项 / 结论） |
| 处理流程 | 拼接全文 → 构造 Prompt（包含按发言人分组要求）→ 调用智谱 GLM-4.5-Air API → 解析返回文本 → 右侧卡片分区显示 → 保存到数据库 |
| 数据库存储 | summaries 表：id、meeting_id（UNIQUE）、content、created_at |
| API 参数 | model: glm-4.5-air, temperature: 0.3, max_tokens: 2000 |
| Prompt 模板 | "你是一位专业的会议纪要助手。请根据以下会议转写内容，生成按发言人分组的结构化摘要，包含：1. 按发言人分组（每个发言人的核心观点和贡献）2. 关键点（3-5 个核心讨论要点，注明涉及的发言人）3. 待办事项（明确标注负责人）4. 结论（共识和决定）\n\n会议内容：\n{transcript}" |
...
| --- | --- |
| 功能名称 | 会议置顶 |
| 触发方式 | 右键点击会议列表项 → 选择"置顶"/"取消置顶" |
| 输入 | 会议 ID |
| 输出 | 更新置顶状态，列表重新排序（置顶会议优先显示） |
| 处理流程 | 点击右键菜单 → toggle_pin_meeting → UPDATE meetings SET is_pinned → 刷新列表 |
| 数据库更新 | `UPDATE meetings SET is_pinned = 1/0 WHERE id = ?` |
| 关键约束 | 置顶会议在列表顶部显示，带 📌 图标；非置顶会议按时间倒序排列 |
| UI 交互 | 右键菜单显示"置顶"或"取消置顶"；置顶项显示图标标识 |
...
| 输入 | 搜索关键词（会议标题或日期） |
| 输出 | 过滤后的会议列表 |
| 处理流程 | 输入关键词 → search_meetings → LIKE 查询标题和时间 → 刷新列表 |
| 数据库查询 | `SELECT * FROM meetings WHERE title LIKE ? OR start_time LIKE ? ORDER BY is_pinned DESC, start_time DESC` |
| 关键约束 | 支持按标题名称搜索；支持按日期搜索（如"2026-06"）；置顶会议优先显示 |
| UI 交互 | 搜索框实时过滤；清空搜索框恢复完整列表 |
...
| 输入 | 用户输入的会议名称 |
| 输出 | 更新会议标题，刷新列表显示 |
| 处理流程 | 会议结束 → 弹出 QInputDialog → 用户输入名称 → update_meeting_title → 刷新列表 |
| 默认名称格式 | 会议--日期--时长（如"第一次大会--2026.06.25--1时30分"） |
| 数据库更新 | `UPDATE meetings SET title = ? WHERE id = ?` |
| 关键约束 | 必须输入名称才能完成结束流程；名称不能为空 |
| UI 交互 | 输入框预填默认名称；用户可修改后确认 |
...
| 触发方式 | 会议列表加载时自动计算 |
| 输入 | 会议开始时间、结束时间 |
| 输出 | 格式化的时长字符串（如"1时30分"、"45分20秒"） |
| 处理流程 | 加载会议 → 计算时间差 → 格式化显示 → 列表项追加时长信息 |
| 计算逻辑 | `duration = end_time - start_time` → 格式化为易读字符串 |
| 关键约束 | 正在进行中的会议显示当前时长；已结束会议显示总时长 |
| UI 交互 | 会议列表项格式：标题 + 时间 + 时长 |
