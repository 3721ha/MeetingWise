...
| 文件 | 说明 |
| --- | --- |
| meetwise.db | SQLite 数据库（会议、发言、摘要等数据） |
| meetwise.log | 运行日志（DEBUG/INFO/WARNING/ERROR 全量记录） |
| recordings/ | 会议录音 WAV 文件 |
| voiceprints/ | 注册声纹特征向量 (.npy 文件) |

日志说明：控制台只显示 WARNING 及以上级别的日志，详细信息保存到 meetwise.log 文件，方便排查问题。
...
1. 左侧面板显示所有历史会议（置顶优先，按时间排序）
2. 列表项显示会议标题、时间、时长
3. 点击查看详情（转写内容、摘要、对话记录）
4. 右键菜单：置顶/取消置顶、删除
...
1. 会议结束时自动弹出命名对话框
2. 默认名称格式："会议--日期--时长"（如"第一次会议--2026.06.25--1时30分"）
3. 可修改名称后确认保存
